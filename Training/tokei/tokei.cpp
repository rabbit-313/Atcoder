...
struct Clock{
    ...
    string to_str(){
        // cout << setw(2) << setfill('0') << endl;
        // printf("%02d:%02d:%02d", hour, miute, second)
    ...
    void shift(int diff_second){
       long long now_time = 3600 * hour + 60 * minute + second;
        now_time += diff_second;

        if (now_time < 0){
            now_time = 86400 + now_time;
        }

        hour = now_time / 3600;
        minute = (now_time - hour * 3600) / 60;
        second = now_time - hour * 3600 - minute * 60;
